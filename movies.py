...
def movie_from_name(tx, name1, name2):
    query = "MATCH (p1:Person{name: $name1})-[]->(m:Movie)<-[]- (p2:Person {name:$name2}) RETURN m.title as Title, m.released as Released, substring(m.tagline, 0, 20) as ShortTagline"
...
if __name__ == "__main__":
    ...
    with neo4jDriver.session() as session:
        ...
        for record in result:
            print((f"Title: {record['Title']}, Released: {record['Released']}, Tagline: {record['ShortTagline']}"))
        neo4jDriver.close()